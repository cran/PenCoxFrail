### control
coxlassoControl<-function(start = NULL, index=NULL, q_start = NULL, conv.eps = 1e-3, 
...
{                       
  list(start = start, index=index, q_start = q_start, conv.eps = conv.eps, standardize = standardize, center = center,
       smooth = smooth, print.iter = print.iter, max.iter = max.iter, c.app =c.app, exact = exact, xr = xr, eps = eps, 
       quant.knots =  quant.knots, ...)
}
