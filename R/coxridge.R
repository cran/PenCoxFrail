...
##############################################################
##############################################################
coxridge<- function(fix=formula, rnd = NULL, vary.coef = NULL, 
                       xi.ridge, data, control = list()){
  
  est <- est.coxridge(fix=fix,rnd=rnd,vary.coef=vary.coef,data=data, xi.ridge = xi.ridge,
                         control=control)
...
est.coxridge <- function(fix, rnd, vary.coef, data, xi.ridge, control = list() )
{
  if(grepl("\\*", fix[3]))
  ...
  ## dummy, whether categrical covariates / groups are present
  ...
  ## ridge penalty on betas
  diag(P)[(dimb+1):(dimb+lin)] <- xi.ridge 
...
summary.coxridge <- function(object, ...)
{
  ...
  class(res) <- "summary.coxridge"
  res
}

##############################

print.summary.coxridge <- function(x, ...)
{
  cat("Call:\n")
  print(x$call)
  cat("\nFixed Effects:\n")
...
plot.coxridge <- function(x,which.comp=NULL,main=NULL,...)
{
  help.list <- list(...)
...
predict.coxridge <- function(object, newdata=NULL, time.grid=NULL, id.var = NULL, ...)
{
  if(is.null(newdata))
    newdata <- object$data 
